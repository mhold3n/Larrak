...
from Simulations.Combustion.prechamber_reduced import CombustionConfig, PrechamberSimulation
...
def test_combustion_pipeline():
    ...
    for rpm, v_pre in configs:
        ...
        inp = SimulationInput(
            run_id=f"comb_{rpm}_{v_pre}", geometry=geo, operating_point=ops, boundary_conditions=bcs
        )
        sim.load_input(inp)
        ...
        print(
            f"Run {rpm} RPM: Jet={out.calibration_params['jet_intensity']:.4f}, m={out.calibration_params['wiebe_m']:.4f}"
        )
