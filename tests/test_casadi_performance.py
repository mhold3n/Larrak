...
from campro.constants import (
    ...
    CASADI_PHYSICS_MAX_GRADIENT_TIME_MS,
)
...
class TestCasadiPerformance:
    ...
    def setup_method(self):
        ...
        self.F_vec = 1000.0 * np.ones(self.n_chunked)  # For torque profile wrapper (numpy)
        self.litvin_config = ca.DM([50.0, 20.0, 2.0, 20.0, 25.0, 0.0])  # Litvin disabled
    ...
    def _measure_gradient_time(self, fn: ca.Function, args: list, wrt_idx: int) -> float:
    ...
    def _measure_torque_profile_time(self, theta_vec: np.ndarray, F_vec: np.ndarray) -> float:
        """Measure torque profile chunked wrapper evaluation time in milliseconds."""
        # Warm up
        for _ in range(3):
            self.torque_profile_fn(theta_vec, F_vec, self.r, self.l, 
    ...
    def test_unified_physics_evaluation_performance(self):
        ...
        args = [
            self.theta_vec, self.pressure_vec, self.r, self.l, 
            self.x_off, self.y_off, self.bore, self.max_side_threshold, 
            self.litvin_config
        ]
    ...
    def test_torque_profile_gradient_performance(self):
        ...
        pytest.skip("Gradient testing not applicable for chunked wrapper (Python function)")
    ...
    def test_performance_consistency(self):
        ...
        assert cv < 0.2, \
            f"Performance too inconsistent: CV={cv:.3f}, times={times}ms"
