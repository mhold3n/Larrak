...
class TestCasadiOptimizerIntegration:
    ...
    def test_unified_physics_evaluation(self):
        ...
        # Evaluate function
        result = unified_fn(theta_vec, pressure_vec, r, l, x_off, y_off, 
    ...
    def test_validation_mode(self):
        ...
        if hasattr(self.optimizer, '_validate_casadi_physics_at_point'):
            ...
            validation_result = self.optimizer._validate_casadi_physics_at_point(
                params, param_names, self.motion_law_data, 
                self.load_profile, self.litvin_geometry
            )
    ...
    def test_no_nan_values_in_outputs(self):
        ...
        if hasattr(result, 'x'):
            assert not np.any(np.isnan(result.x)), "Optimization result contains NaN values"
