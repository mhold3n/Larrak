...
class TestThermalEfficiencyIntegration:
    ...
    def test_thermal_efficiency_no_nan(self):
        ...
        try:
            ...
            if hasattr(result, 'success') and result.success:
                if hasattr(result, 'solution') and result.solution is not None:
                    ...
                    if hasattr(result.solution, 'x'):
                        assert not np.any(np.isnan(result.solution.x)), "Solution contains NaN values"
                    if hasattr(result.solution, 'v'):
                        assert not np.any(np.isnan(result.solution.v)), "Velocity contains NaN values"
                    if hasattr(result.solution, 'a'):
                        assert not np.any(np.isnan(result.solution.a)), "Acceleration contains NaN values"
                
                # Check objective value
                if hasattr(result, 'objective_value'):
                    assert not np.isnan(result.objective_value), "Objective value is NaN"
            ...
            assert "invalid_number_detected" not in error_msg, f"Optimization failed with Invalid_Number_Detected: {e}"
    ...
    def test_thermal_efficiency_with_different_motion_types(self):
        ...
        for motion_type in motion_types:
            try:
                ...
                if hasattr(result, 'success') and result.success:
                    if hasattr(result, 'objective_value'):
                        assert not np.isnan(result.objective_value), f"Objective value is NaN for {motion_type}"
        ...
        try:
            ...
            if hasattr(result, 'success') and result.success:
                if hasattr(result, 'solution') and result.solution is not None:
                    ...
                    if hasattr(result.solution, 'x'):
                        stroke_achieved = np.max(result.solution.x) - np.min(result.solution.x)
                    ...
                    if hasattr(result.solution, 'a'):
                        ...
                        assert max_acceleration <= self.constraints.max_acceleration * 1.1, \
    ...
    def test_thermal_efficiency_performance(self):
        ...
        try:
            ...
            assert optimization_time < 60.0, f"Optimization took too long: {optimization_time:.2f}s"
