...
from truthmaker.cem import CEMClient, OperatingRegime, ValidationReport

from ..models.ensemble import EnsembleSurrogate
from ..sampling.collector import CEMDataCollector
...
class GatedSurrogateInference:
    ...
    def predict(
        ...
        if regime.value not in self.valid_regimes:
            return self._call_fallback(x_array, f"Invalid regime: {regime.name}", regime.value)
    ...
    def _call_fallback(self, x: np.ndarray, reason: str, regime_id: int) -> SurrogatePrediction:
    ...
    def _log_for_retraining(self, x: np.ndarray, y: np.ndarray, regime_id: int) -> None:
