...
class ResultProcessor:
    ...
    def __init__(self, config: OptimizationConfig):
        self.config = config
        # Pass configuration mapping to validators (required by consolidated validators)
        try:
            config_map = asdict(self.config)
        except Exception:
            config_map = {}
        self.solution_validator = SolutionValidator(config_map)
        self.physics_validator = PhysicsValidator(config_map)
