...
def find_hsl_library(env_path: Path | None = None) -> Optional[Path]:
    ...
    # Fallback: Check project CoinHSL archive folder (Windows-specific)
    if system == "windows":
        try:
            # Try to detect project root
            import campro
        ...
        # Check for specific CoinHSL archive folder
        ...
        if hsl_folder.exists():
            ...
            if dll.exists():
                log.info(f"Found HSL library in project CoinHSL archive: {dll}")
                return dll
