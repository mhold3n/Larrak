...
_AVAILABLE_SOLVERS_CACHE: Optional[list[str]] = None
_COINHSL_DIRECTORY_CACHE: Optional[Path] = None
...
def find_coinhsl_directory(project_root: Path | None = None) -> Optional[Path]:
    """
    ...
    # Search for CoinHSL.v* directories (not CoinHSL-archive.*)
    ...
    if not platform_matches:
        log.debug(f"No CoinHSL directories found matching platform (Windows={IS_WINDOWS}, macOS={IS_MACOS})")
        return None
...
def get_hsl_library_path(coinhsl_dir: Path | None = None) -> Optional[Path]:
    """
...
def detect_available_solvers(
    ...
    potentially_available = [
        solver for solver in all_solvers
        if compile_time_available.get(solver, False)
    ]
    ...
    if not available_solvers:
        ...
        log.info(f"Detected {len(available_solvers)} available HSL solvers: {', '.join(available_solvers)}")
