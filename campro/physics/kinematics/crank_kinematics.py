...
class CrankKinematics(BasePhysicsModel):
    ...
    def simulate(self,
        ...
        try:
            ...
            angular_velocity = inputs.get("angular_velocity", 100.0)  # Default 100 rad/s
            ...
            kinematics_result = self._compute_kinematics_analysis(
                motion_law_data, crank_center_offset, angular_velocity,
            )
            ...
            data = {
                ...
                "max_piston_velocity": np.array([kinematics_result.max_piston_velocity]),
                "max_piston_acceleration": np.array([kinematics_result.max_piston_acceleration]),
            }
    ...
    def compute_rod_angle(self, crank_angle: float, crank_center_offset: Tuple[float, float]) -> float:
        """
        ...
        effective_crank_angle = crank_angle + np.arctan2(crank_center_offset[1], crank_center_offset[0])

        rod_angle = np.arcsin((self._crank_radius * np.sin(effective_crank_angle)) / self._rod_length)

        return rod_angle

    def compute_rod_angular_velocity(self,
                                   crank_angle: float,
        ...
        denominator = np.sqrt(1 - (self._crank_radius * sin_theta / self._rod_length)**2)
        if denominator == 0:
            # Handle singularity case
            rod_angular_velocity = 0.0
        else:
            dphi_dtheta = (self._crank_radius * cos_theta) / (self._rod_length * denominator)
            rod_angular_velocity = dphi_dtheta * crank_angular_velocity

        return rod_angular_velocity

    def compute_corrected_piston_motion(self,
                                      motion_law_data: Dict[str, np.ndarray],
                                      crank_center_offset: Tuple[float, float]) -> Dict[str, np.ndarray]:
        """
        ...
        for i, crank_angle in enumerate(crank_angles):
            ...
            offset_correction = (crank_center_offset[0] * np.cos(crank_angle) +
            ...
            if i > 0:
                ...
                corrected_velocities[i] = (corrected_displacements[i] - corrected_displacements[i-1]) / dt
            else:
            ...
            if i > 1:
                dt = crank_angle - crank_angles[i-1]
                corrected_accelerations[i] = (corrected_velocities[i] - corrected_velocities[i-1]) / dt
            else:
    ...
    def _compute_kinematics_analysis(self,
                                   motion_law_data: Dict[str, np.ndarray],
                                   crank_center_offset: Tuple[float, float],
                                   angular_velocity: float) -> CrankKinematicsResult:
        ...
        for i, crank_angle in enumerate(crank_angles):
            ...
            rod_angular_velocities[i] = self.compute_rod_angular_velocity(
                crank_angle, angular_velocity, crank_center_offset,
            )

        # Compute rod angular accelerations (numerical derivative)
        for i in range(1, n_points):
            dt = crank_angles[i] - crank_angles[i-1]
            rod_angular_accelerations[i] = (rod_angular_velocities[i] - rod_angular_velocities[i-1]) / dt

        # Compute corrected piston motion
        corrected_motion = self.compute_corrected_piston_motion(motion_law_data, crank_center_offset)
