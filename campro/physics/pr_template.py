...
def compute_pr_template(
    ...
    *,
    ...
    if combustion_start_wrapped > combustion_end_wrapped:
        ...
        combustion_phase = (theta_deg >= combustion_start_wrapped) | (theta_deg < combustion_end_wrapped)
    else:
        combustion_phase = (theta_deg >= combustion_start_wrapped) & (theta_deg < combustion_end_wrapped)
    ...
    if np.any(evo_phase):
        ...
        pr_evo = pr_evo_start * np.exp(-3.0 * evo_distance_norm) + pr_ambient * (1.0 - np.exp(-3.0 * evo_distance_norm))
