...
def _import_casadi() -> Any:
    """Import CasADi or raise ImportError."""
    if ca is None:
        raise ImportError("CasADi is not installed or could not be imported.")
    return ca
...
def indicated_work_simpson(*, p_series: list[float], V_series: list[float]) -> float:
    ...
    if n % 2 == 0:
        ...
        if n > 2:
            ...
            W_ind += (h / 8.0) * (
                p_series[n - 3] + 3.0 * p_series[n - 2] + 3.0 * p_series[n - 1] + p_series[n]
            )
    else:
...
def cycle_analysis(
    *,
    ...
    # Thermal efficiency
    eta_th = (
        thermal_efficiency(W_ind=W_ind, Q_in=Q_in) if params.calculate_thermal_efficiency else 0.0
    )
...
def indicated_work_surrogate(
    *,
    p_series: Iterable[float],
    dV_series: Iterable[float],
) -> float:
...
def scavenging_phase_timing_objectives(
    ...
    if "A_in" in controls:
        ...
        objectives["intake_timing"] = (
            weights.get("intake_timing", 1.0) * (t_intake_duration - t_intake_target) ** 2
        )
    ...
    if "A_ex" in controls:
        ...
        objectives["exhaust_timing"] = (
            weights.get("exhaust_timing", 1.0) * (t_exhaust_duration - t_exhaust_target) ** 2
        )
