...
from campro.physics.thermal_efficiency_simple import SimplifiedThermalModel, ThermalEfficiencyConfig
...
class CasADiUnifiedFlow:
    ...
    def __init__(self, settings: Optional[CasADiOptimizationSettings] = None):
        ...
        log.info(f"Initialized CasADiUnifiedFlow: "
    ...
    def optimize_phase1(self, constraints: Dict[str, Any], 
        ...
        **kwargs
        ...
        try:
            ...
            if result.successful:
                ...
                log.info(f"Phase 1 optimization completed: "
    ...
    def _create_problem_from_constraints(self, constraints: Dict[str, Any],
                                       targets: Dict[str, Any]) -> CasADiMotionProblem:
        ...
        problem = CasADiMotionProblem(
            ...
            objectives=targets.get('objectives', ['minimize_jerk', 'maximize_thermal_efficiency']),
            weights=weights,
            n_segments=self.settings.n_segments,
            poly_order=self.settings.poly_order,
            collocation_method=self.settings.collocation_method,
    ...
    def _store_solution(self, problem: CasADiMotionProblem, 
    ...
    def optimize_with_fallback(self, constraints: Dict[str, Any],
                             targets: Dict[str, Any],
                             **kwargs) -> OptimizationResult:
        """
    ...
    def benchmark_optimization(self, problem_specs: list) -> Dict[str, Any]:
        ...
        benchmark_stats = {
            ...
            'avg_solve_time': np.mean([r['solve_time'] for r in successful_results]) if successful_results else 0,
            'min_solve_time': np.min([r['solve_time'] for r in successful_results]) if successful_results else 0,
            'max_solve_time': np.max([r['solve_time'] for r in successful_results]) if successful_results else 0,
            'avg_efficiency': np.mean([r['efficiency'] for r in successful_results]) if successful_results else 0,
            'results': results
        }
        
        log.info(f"Benchmark completed: {benchmark_stats['success_rate']:.1%} success rate, "
