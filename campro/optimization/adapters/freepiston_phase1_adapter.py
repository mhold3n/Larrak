...
class FreePistonPhase1Adapter(BaseOptimizer):
        ...
        **kwargs,
        ...
        if isinstance(constraints, CamMotionConstraints):
            ...
            return self.solve_cam_motion_law(
                ...
                disable_combustion=kwargs.get("disable_combustion", False),
            )
        else:
    ...
    def solve_custom_objective(
        ...
        **kwargs,
        ...
        log.info(
            "solve_custom_objective: delegating to solve_cam_motion_law "
            "(custom objective not used by free-piston IPOPT flow)"
        )
    ...
    def solve_cam_motion_law(
        ...
        *,
        ...
        try:
            ...
            disable_combustion_attr = getattr(self, "_disable_combustion", False)
            ...
            P = self._build_problem_dict(
                ...
                disable_combustion=disable_combustion or disable_combustion_attr,
            )
            ...
            # Determine if fuel continuation is needed
            combustion_cfg = P.get("combustion", {})
            fuel_mass = combustion_cfg.get("fuel_mass_kg", 0.0)
            use_continuation = fuel_mass > 1e-10  # Use continuation if fuel > 0

            # Check for override via environment variable
            if os.getenv("FREE_PISTON_DISABLE_CONTINUATION", "0") == "1":
                use_continuation = False
                reporter.info("Fuel continuation disabled via environment variable")

            # Solve using appropriate strategy
            solve_start = time.time()
            if use_continuation:
                reporter.info(f"Using fuel continuation (target fuel={fuel_mass:.2e} kg)")
                from campro.optimization.driver import (
                    solve_cycle_with_fuel_continuation,
                )

                solution = solve_cycle_with_fuel_continuation(P)
            else:
                reporter.info("Solving directly (no continuation)")
                solution = solve_cycle(P)
            solve_elapsed = time.time() - solve_start
            reporter.info(
                f"Free-piston IPOPT solve_cycle() completed in {format_duration(solve_elapsed)}"
            )
            ...
            if diag_enabled:
                ...
                with reporter.section("Optimization diagnostics", level="INFO"):
                    ...
                    if ca_markers:
                        reporter.info(
                            "CA markers: "
                            + ", ".join(f"{k}={v:.1f}°" for k, v in ca_markers.items())
                        )
    ...
    def _build_problem_dict(
        ...
        # Update bounds to match stroke
        # Free piston bounds are critical for the solver and initial guess
        config.bounds["xR_max"] = stroke_m
        config.bounds["xR_min"] = 0.0
        config.bounds["xL_max"] = 0.0
        config.bounds["xL_min"] = -stroke_m

        # Also update velocity bounds based on expected speed
        # Mean piston speed ~ 2 * stroke / cycle_time
        # Max speed ~ pi * mean approx 6 * stroke / cycle_time
        v_est = 10.0 * stroke_m / max(cycle_time, 1e-3)
        config.bounds["v_max"] = v_est
        config.bounds["v_min"] = -v_est
        ...
        config.num = {
            "K": K,
            "C": 1,
            "min_segments": min_segments_val,
            "refinement_factor": refinement_factor_val,
        }  # Radau collocation with C=1
        ...
        # When combustion is disabled, we must disable efficiency and scavenging objectives
        # because Q_in becomes zero (leading to division by zero in efficiency)
        # and scavenging variables might be disabled or irrelevant.
        if disable_combustion:
            config.objective["w"] = {
                "eta_th": 0.0,
                "short_circuit": 0.0,
                "smoothness": config.objective.get("w", {}).get("smoothness", 1.0),
            }
            msg = "Objectives disabled: eta_th=0.0, short_circuit=0.0 (physics debugging mode)"
            log.info(msg)
            msg = "Objectives disabled: eta_th=0.0, short_circuit=0.0 (physics debugging mode)"
            log.info(msg)
        ...
        # Inject solver options from config if available
        # This allows overriding max_iter, tol, etc. from the adapter configuration
        solver_options = self._config.get("solver_options", {})
        if solver_options:
            if "ipopt" in solver_options:
                config.solver["ipopt"].update(solver_options["ipopt"])
            # Also support direct keys like "max_iter" for convenience
            if "max_iter" in solver_options:
                config.solver["ipopt"]["ipopt.max_iter"] = int(solver_options["max_iter"])
            if "tol" in solver_options:
                config.solver["ipopt"]["ipopt.tol"] = float(solver_options["tol"])
    ...
    def _convert_solution_to_result(
        ...
        variable_groups = (
            (meta_nlp.get("variable_groups") or {}) if isinstance(meta_nlp, dict) else {}
        )
        ...
        use_freepiston_motion = (
            os.environ.get("PHASE1_USE_FREEPISTON_MOTION", "1").lower() not in _FALSEY
        )

        try:
            if (
                use_freepiston_motion
                and isinstance(variable_groups, dict)
                and len(variable_groups) > 0
            ):
                x_opt_vec = np.asarray(x_opt, dtype=float).flatten()

                # Build combustion-driven time grid
                comb_meta = (
                    (meta_nlp.get("combustion_model") or {}) if isinstance(meta_nlp, dict) else {}
                )
                omega_deg_per_s = float(
                    comb_meta.get(
                        "omega_deg_per_s",
                        360.0 / max(float(self._last_cycle_time or cycle_time), 1e-9),
                    )
                )
                ...
                # Expect sizes: 2 (initial) + K*(C+1)*2 for positions/velocities due to boundary vars
                # We need to filter out the boundary variables to match the time grid (1 + K*C)

                def _extract_filtered_pairs(
                    idx_list: list[int],
                    ...
                    n_pairs = reshaped.shape[0]

                    # Filter logic: Keep pair p if p=0 (initial) or (p-1) % (C+1) < C (collocation)
                    # Boundary points (where (p-1) % (C+1) == C) are redundant/duplicates of next initial
                    keep_indices = []
                    ...
                    filtered = reshaped[keep_indices]
                    return filtered[:, 0], filtered[:, 1]

                # Robust extraction with validation
                xL_all, xR_all = _extract_filtered_pairs(pos_idx)

                # Validation: Position should be small (e.g. < 2.0m)
                # If values are large (e.g. > 100), indices are likely wrong (pointing to T or P)
                is_valid = False
                if xL_all.size > 0:
                    # Check magnitude of first point
                    mag = np.max(np.abs(xL_all))
                    if mag < 2.0:  # 2.0m is huge for an engine, but safe upper bound
                        is_valid = True
                    else:
                        log.warning(
                            f"Position indices {pos_idx[:2]} point to large values (mag={mag:.2e}). likely incorrect."
                        )

                if not is_valid and x_opt_vec.size > 0:
                    log.warning("Scanning x_opt for plausible position variables...")
                    # Heuristic: xL is negative small, xR is positive small

                    candidates = []
                    for i in range(len(x_opt_vec) - 1):
                        val1 = x_opt_vec[i]
                        val2 = x_opt_vec[i + 1]

                        # Check if val1 is neg, val2 is pos, and both small
                        # Relaxed symmetry check: just check signs and magnitude
                        if val1 < -0.001 and val2 > 0.001 and abs(val1) < 2.0 and abs(val2) < 2.0:
                            # Found a potential pair
                            candidates.append(i)

                    if candidates:
                        # Assume the first candidate block corresponds to positions
                        # We need K*C points (plus initial)
                        # Just take the first one found as start
                        start_idx = candidates[0]
                        # Try to reconstruct pos_idx from this start
                        # Assuming contiguous pairs? No, they are interleaved with other vars usually?
                        # Or maybe they are grouped?
                        # In collocation NLP, variables are usually grouped by type or by point.
                        # If grouped by type (which variable_groups implies), then they should be contiguous.

                        # Let's assume they are contiguous for now if variable_groups failed
                        # But variable_groups says they are at pos_idx.

                        # If pos_idx points to large values, maybe the scaling is wrong?
                        # Or maybe the indices are wrong.

                        if len(candidates) > K:
                            # Try to find a consistent stride
                        ...
                        if len(candidates) > K:
                            ...
                            if len(counts) > 0:
                                common_stride = unique[np.argmax(counts)]

                                # Filter candidates that match this stride
                                filtered_candidates = [candidates[0]]
                                for c in candidates[1:]:
                                    if (c - filtered_candidates[-1]) == common_stride:
                                        filtered_candidates.append(c)

                                # Reconstruct pos_idx from filtered candidates
                                new_pos_idx = []
                                for c in filtered_candidates:
                                    new_pos_idx.append(c)
                                    new_pos_idx.append(c + 1)

                                log.info(f"Reconstructed pos_idx from scan: {new_pos_idx[:10]}...")
                                xL_all, xR_all = _extract_filtered_pairs(new_pos_idx)
                    else:
                        log.error("Could not find plausible position variables in x_opt")

                vL_all, vR_all = _extract_filtered_pairs(vel_idx)
                ...
                for k in range(K):
                    t_start = k * dt_elem
                    t_colloc_list.append(t_start + dt_elem * nodes)
                t_colloc = np.concatenate(t_colloc_list) if t_colloc_list else np.array([])
                t = np.concatenate([[0.0], t_colloc])

                # Assemble follower motion (half of total expansion = per-piston motion)
                # xR is positive, xL is negative. Expansion increases xR and decreases xL.
                # Relative separation is xR - xL. Per-piston travel is (xR - xL) / 2.
                x_follow_m = np.concatenate([[0.5 * (xR0 - xL0)], 0.5 * (xR_colloc - xL_colloc)])

                v_follow_mps = np.concatenate([[0.5 * (vR0 - vL0)], 0.5 * (vR_colloc - vL_colloc)])

                # Ensure strictly monotonic time grid to prevent divide-by-zero in gradient
                # Filter out duplicate or very close time points
                min_dt = 1e-9  # Minimum time step (1 nanosecond)
                unique_mask = np.concatenate([[True], np.diff(t) > min_dt])
                if not unique_mask.all():
                    log.debug(
                        f"Filtered {(~unique_mask).sum()} duplicate/close time points "
                        f"(min_dt={min_dt:.2e}s) to ensure monotonic grid for gradient"
                    )
                    t = t[unique_mask]
                    x_follow_m = x_follow_m[unique_mask]
                    v_follow_mps = v_follow_mps[unique_mask]

                # Compute acceleration using gradient (now safe from divide-by-zero)
                a_follow_mps2 = (
                    np.gradient(v_follow_mps, t, edge_order=2)
                    if t.size >= 3
                    else np.zeros_like(v_follow_mps)
                )
                ...
                def _filter_redundant_scalars(vals: np.ndarray) -> np.ndarray:
                    if vals.size < 2:
                        return vals
                    n_vals = vals.size
                    keep_indices = []
                    for p in range(n_vals):
                        if p == 0:
                            ...
                            keep_indices.append(p)
                    return vals[keep_indices]

                # Expect 1 + K*C values each (after filtering)
                if rho_vals.size >= 1 and T_vals.size >= 1:
                    rho_series = _filter_redundant_scalars(np.asarray(rho_vals, dtype=float))
                    T_series = _filter_redundant_scalars(np.asarray(T_vals, dtype=float))

                    # Ensure lengths match the filtered time grid (not original 1+K*C)
                    target_len = len(t)  # Use actual filtered time grid length
                    if rho_series.size > target_len:
                        rho_series = rho_series[:target_len]
                    if T_series.size > target_len:
                        T_series = T_series[:target_len]

                    # Pad if needed (shouldn't happen with proper filtering)
                    if rho_series.size < target_len:
                        rho_series = np.pad(
                            rho_series, (0, target_len - rho_series.size), mode="edge"
                        )
                    if T_series.size < target_len:
                        T_series = np.pad(T_series, (0, target_len - T_series.size), mode="edge")
            ...
            try:
                ...
                stroke_m = (
                    float(self._last_geometry.get("stroke_mm", cam_constraints.stroke)) / 1000.0
                )
                    ...
                    for c in load_sweep:
                        ...
                        if combustion_inputs is not None:
                            ...
                            if "fuel_mass" in comb_inputs_case:
                                comb_inputs_case["fuel_mass"] = float(
                                    comb_inputs_case["fuel_mass"]
                                ) * float(fm)
                        ...
                        p_cyl_case = np.asarray(
                            eval_out_case.get("p_cyl") or eval_out_case.get("p_comb"),
                            dtype=float,
                        )
                ...
                p_cyl_eval = np.asarray(
                    eval_out.get("p_cyl") or eval_out.get("p_comb"), dtype=float
                )
