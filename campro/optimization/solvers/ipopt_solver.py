...
class IPOPTOptions:
    ...
    max_iter: int = 10000  # Increased default from 3000 to ensure convergence on stiff problems
    ...
    # Scaling options
    nlp_scaling_method: str = "gradient-based"  # "gradient-based", "none", "equilibration-based"
    nlp_scaling_max_gradient: float = 100.0

    # MA57 specific options
    ma57_automatic_scaling: str = "yes"
    ma57_pre_alloc: float = 3.0
    ...
    expect_infeasible_problem: str = (
        "no"  # "no" = disabled (scaling should make problems more feasible)
    )
    ...
    # Plateau detection options
    plateau_check_enabled: bool = True  # Default to True as requested
    plateau_eps: float = 1e-4  # Minimum relative change to call it progress
    plateau_window_size: int = 20  # Number of recent iterations to look at
    plateau_start_iter: int = 100  # Iteration to begin checks
...
class IPOPTIterationCallback(ca.Callback):
    ...
    def __init__(
        ...
        plateau_window_size: int = 20,
        plateau_start_iter: int = 100,
    ) -> None:
        ...
        self._plateau_enabled = plateau_enabled
        self._plateau_eps = plateau_eps
        self._plateau_window_size = plateau_window_size
        self._plateau_start_iter = plateau_start_iter
        self._plateau_buffer: list[float] = []
    ...
    def get_sparsity_out(self, idx: int) -> ca.Sparsity:
    ...
    def eval(self, args: list[Any]) -> list[int]:
        ...
        try:
            ...
            lambda_inf = float(np.max(np.abs(lam_g))) if lam_g is not None and lam_g.size else 0.0
            ...
            # Divergence Guardrail (Prevent Segfaults)
            # If Lagrange multipliers or constraints explode, stop early.
            # NOTE: Threshold relaxed from 1e12 to 1e15 to handle stiff dynamics tuning.
            if lambda_inf > 1e15 or primal_violation > 1e6:
                self._reporter.warning(
                    f"DIVERGENCE DETECTED at iter {self._iteration}: "
                    f"|lam_g|_inf={lambda_inf:.2e}, g_viol={primal_violation:.2e}. "
                    "Stopping optimization to prevent crash."
                )
                return [1]

            # Plateau detection logic
            if self._plateau_enabled:
                stop_optimization = self._check_plateau(
                    iteration=self._iteration,
                    constr_viol=primal_violation,
                    dual_inf=lambda_inf,
                    compl_inf=0.0,  # Not easily available in callback without more work
                )
    ...
    def _check_plateau(
        ...
        """
        Check for plateau conditions and return True if optimization should stop.

        A plateau is detected if the relative change in the quality metric Q_k
        is small over a window of iterations, indicating diminishing returns.
        ...
        if self._last_Q is not None:
            ...
            # Check condition
            if (
                iteration >= self._plateau_start_iter
                and len(self._plateau_buffer) >= self._plateau_window_size
            ):
                max_delta = max(self._plateau_buffer)
                if max_delta < self._plateau_eps:
                    # Identify the dominating metric
                    metrics = {
                        "Constraint Violation": constr_viol,
                        "Dual Infeasibility": dual_inf,
...
class IPOPTSolver:
    ...
    def solve(
        ...
        """
        # Store NLP and metadata for diagnostic error reporting
        self._nlp_for_diagnostics = nlp
        # Try to extract metadata if available (from driver)
        self._meta_for_diagnostics = getattr(nlp, "meta", None) if hasattr(nlp, "meta") else None
        if not self.ipopt_available:
            raise RuntimeError(
        ...
        try:
            ...
            trace_enabled = _env_flag(
                "FREE_PISTON_IPOPT_TRACE", default=True
            )  # Default to True for verbose output
            if trace_enabled:
                step_raw = os.environ.get(
                    "FREE_PISTON_IPOPT_TRACE_STEP", "1"
                )  # Default to every iteration
                try:
                ...
                iteration_callback = IPOPTIterationCallback(
                    ...
                    n_params=n_params,
                    step=callback_step,
                    plateau_enabled=self.options.plateau_check_enabled,
                    plateau_eps=self.options.plateau_eps,
                    plateau_window_size=self.options.plateau_window_size,
                    plateau_start_iter=self.options.plateau_start_iter,
                )
                reporter.debug(
            ...
            # Debug print
            print(f"DEBUG: iteration_callback created: {iteration_callback is not None}")
            if iteration_callback:
                print(f"DEBUG: plateau_enabled={iteration_callback._plateau_enabled}")
                print(f"DEBUG: plateau_eps={iteration_callback._plateau_eps}")
            # Create IPOPT solver
            ...
            try:
                ...
                if "Invalid_Number" in error_str or "NaN" in error_str or "Inf" in error_str:
                    ...
                    try:
                        ...
                        if (
                            hasattr(self, "_nlp_for_diagnostics")
                            and self._nlp_for_diagnostics is not None
                        ):
                            ...
                            if isinstance(nlp_dict, dict) and "g" in nlp_dict and "x" in nlp_dict:
                                ...
                                if len(nan_rows) > 0:
                                    reporter.error(
                                        f"NaN detected in constraints at {len(nan_rows)} row(s):"
                                    )
                                    for row_idx in nan_rows[:10]:
                                        reporter.error(f"  Row {row_idx}: g[{row_idx}] = NaN")
                                    if len(nan_rows) > 10:
                                        reporter.error(
                                            f"  ... and {len(nan_rows) - 10} more rows with NaN"
                                        )

                                if len(inf_rows) > 0:
                                    reporter.error(
                                        f"Inf detected in constraints at {len(inf_rows)} row(s):"
                                    )
                                    ...
                                    if len(inf_rows) > 10:
                                        reporter.error(
                                            f"  ... and {len(inf_rows) - 10} more rows with Inf"
                                        )
                                ...
                                try:
                                    ...
                                    if len(jac_nan_pairs[0]) > 0:
                                        reporter.error(
                                            f"NaN detected in Jacobian at {len(jac_nan_pairs[0])} entry/entries"
                                        )
                                        # Get metadata if available for better diagnostics
                                        meta = None
                                        if hasattr(self, "_meta_for_diagnostics"):
                                        ...
                                        for i in range(min(5, len(jac_nan_pairs[0]))):
                                            row_idx = int(jac_nan_pairs[0][i])
                                            col_idx = int(jac_nan_pairs[1][i])
                                            ...
                                            try:
                                                ...
                                                if diag.get("variable_value") is not None:
                                                    reporter.error(
                                                        f"    Variable value: {diag.get('variable_value')}"
                                                    )
                                            except Exception as diag_exc:
                                                reporter.debug(
                                                    f"    Could not diagnose NaN location: {diag_exc}"
                                                )

                                        if len(jac_nan_pairs[0]) > 5:
                                            reporter.error(
                                                f"  ... and {len(jac_nan_pairs[0]) - 5} more NaN entries"
                                            )

                                    if len(jac_inf_pairs[0]) > 0:
                                        reporter.error(
                                            f"Inf detected in Jacobian at {len(jac_inf_pairs[0])} entry/entries"
                                        )
                    ...
                    reporter.error("=" * 80)
                    reporter.error(
                        "Enable diagnostic mode (FREE_PISTON_DIAGNOSTIC_MODE=1) for more details"
                    )
                    reporter.error("=" * 80)
    ...
    def _compute_kkt_error(
        ...
        try:
            # Check for inf/nan in multipliers
            if np.any(np.isnan(lambda_opt)) or np.any(np.isinf(lambda_opt)):
                return float("inf")
            ...
            # Check gradient
            if np.any(np.isnan(grad_f)) or np.any(np.isinf(grad_f)):
                return float("inf")
            ...
            # Check Jacobian
            if np.any(np.isnan(jac_g)) or np.any(np.isinf(jac_g)):
                return float("inf")

            # Compute KKT residual with overflow protection
            # Split into chunks if multipliers are very large to avoid overflow
            max_lambda = np.max(np.abs(lambda_opt))
            if max_lambda > 1e100:  # Extremely large multipliers
                return float("inf")

            jac_lambda = jac_g.T @ lambda_opt

            # Check for overflow in matrix-vector product
            if np.any(np.isnan(jac_lambda)) or np.any(np.isinf(jac_lambda)):
                return float("inf")

            kkt_residual = grad_f - jac_lambda

            # Check residual before norm
            if np.any(np.isnan(kkt_residual)) or np.any(np.isinf(kkt_residual)):
                return float("inf")

            kkt_error = np.linalg.norm(kkt_residual)

            # Final check on error value
...
def get_default_ipopt_options() -> IPOPTOptions:
    ...
    # Optimize for OP engine problems with relaxed tolerances for faster convergence
    options.max_iter = 5000
    options.tol = 1e-6  # Relaxed from 1e-8 for faster convergence
    options.acceptable_tol = 1e-5  # Relaxed from 1e-6 for acceptable convergence
    options.constr_viol_tol = 1e-6  # Relaxed from 1e-8 for practical feasibility
    options.dual_inf_tol = 1e-4  # Relaxed for faster convergence
    ...
    options.print_level = 5  # Reduced from 8 for less verbosity
...
def get_robust_ipopt_options() -> IPOPTOptions:
    ...
    options.print_level = (
        8  # Verbose output: includes detailed iteration info and convergence diagnostics
    )
