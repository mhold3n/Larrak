...
class WarmStartManager:
    ...
    def __init__(self, max_history: int = 50, tolerance: float = 0.1,
        ...
        log.info(f"Initialized WarmStartManager: max_history={max_history}, "
                f"tolerance={tolerance}, storage_path={storage_path}")
    
    def store_solution(self, problem_params: Dict[str, Any], 
        ...
        record = SolutionRecord(
            ...
            compression_ratio_limits=problem_params.get('compression_ratio_limits', (20.0, 70.0)),
        ...
        log.debug(f"Stored solution: stroke={record.stroke}, "
                 f"cycle_time={record.cycle_time}, solve_time={record.solve_time:.3f}s")
    
    def get_initial_guess(self, problem_params: Dict[str, Any]) -> Optional[Dict[str, np.ndarray]]:
        """
        ...
        if closest_record:
            log.debug(f"Found closest solution for warm-start: "
        ...
        if len(bracketing_solutions) >= 2:
            log.debug(f"Found {len(bracketing_solutions)} bracketing solutions for interpolation")
            return self._interpolate_between_solutions(bracketing_solutions, problem_params)
    ...
    def _find_closest_solution(self, problem_params: Dict[str, Any]) -> Optional[SolutionRecord]:
    ...
    def _find_bracketing_solutions(self, problem_params: Dict[str, Any]) -> List[SolutionRecord]:
        ...
        sorted_solutions = sorted(self.solution_history, 
                                key=lambda r: (r.stroke, r.cycle_time))
    ...
    def _interpolate_solution(self, record: SolutionRecord, 
                            problem_params: Dict[str, Any]) -> Dict[str, np.ndarray]:
    ...
    def _interpolate_between_solutions(self, solutions: List[SolutionRecord],
                                     problem_params: Dict[str, Any]) -> Dict[str, np.ndarray]:
    ...
    def _generate_fallback_guess(self, problem_params: Dict[str, Any]) -> Dict[str, np.ndarray]:
        ...
        for i in range(n_points):
            if i <= upstroke_index:
                ...
                velocity[i] = -stroke * (6 * phase - 6 * phase**2) / (cycle_time - upstroke_time)
                acceleration[i] = -stroke * (6 - 12 * phase) / ((cycle_time - upstroke_time)**2)
        
        # Jerk (derivative of acceleration)
        for i in range(n_points - 1):
            jerk[i] = (acceleration[i+1] - acceleration[i]) / (cycle_time / (n_points - 1))
    ...
    def save_history(self) -> None:
        ...
        log.debug(f"Saved {len(self.solution_history)} solutions to {self.storage_path}")
    ...
    def load_history(self) -> None:
        ...
        try:
            ...
            self.solution_history = [SolutionRecord.from_dict(data) for data in history_data]
            log.debug(f"Loaded {len(self.solution_history)} solutions from {self.storage_path}")
