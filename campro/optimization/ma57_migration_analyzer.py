...
class MA57MigrationAnalyzer:
                ...
                with open(self.data_file) as f:
                    ...
                    self.data_points = [
                        MigrationDataPoint(
                            ...
                            ma57_report=MA57ReadinessReport(**dp["ma57_report"])
                            if dp.get("ma57_report")
                            else None,
    ...
    def update_with_ma57_run(
        ...
        self._save_data()
        log.info(
            f"Updated data point with MA57 results (speedup: {performance_improvement})",
        )
    ...
    def analyze_migration_readiness(self) -> MigrationAnalysis:
        ...
        ma57_beneficial_runs = len(
            [dp for dp in ma57_runs if self._is_ma57_beneficial(dp)],
        )

        # Calculate average speedup
        speedups = [
            dp.performance_improvement
            for dp in ma57_runs
            if dp.performance_improvement is not None
        ]
        average_speedup = sum(speedups) / len(speedups) if speedups else None

        # Count convergence improvements
        convergence_improvements = len(
            [dp for dp in ma57_runs if dp.convergence_improvement is True],
        )
    ...
    def _is_ma57_beneficial(self, data_point: MigrationDataPoint) -> bool:
        ...
        if (
            data_point.performance_improvement
            and data_point.performance_improvement > 1.2
        ):
            return True
    ...
    def _analyze_by_problem_size(self) -> dict[str, Any]:
        ...
        medium_problems = [
            dp for dp in self.data_points if 100 <= sum(dp.problem_size) < 500
        ]
        ...
        def analyze_group(
            ...
            beneficial = len(
                [dp for dp in ma57_problems if self._is_ma57_beneficial(dp)],
            )
            speedups = [
                dp.performance_improvement
                for dp in ma57_problems
                if dp.performance_improvement is not None
            ]
            ...
            return {
                ...
                "beneficial_percentage": beneficial / len(ma57_problems)
                if ma57_problems
                else 0,
    ...
    def _generate_recommendations(
        ...
        beneficial_percentage = (
            ma57_beneficial_runs / total_runs if total_runs > 0 else 0
        )

        if beneficial_percentage > 0.7:
            recommendations.append(
                "Strong evidence for MA57 migration - high benefit rate",
            )
        elif beneficial_percentage > 0.4:
            recommendations.append(
                "Moderate evidence for MA57 migration - consider selective adoption",
            )
        else:
            recommendations.append(
                "Limited evidence for MA57 migration - investigate specific use cases",
            )

        if average_speedup and average_speedup > 1.5:
            recommendations.append(
                f"Significant performance improvement expected (avg {average_speedup:.1f}x speedup)",
            )
        elif average_speedup and average_speedup > 1.2:
            recommendations.append(
                f"Moderate performance improvement expected (avg {average_speedup:.1f}x speedup)",
            )

        if convergence_improvements > 0:
            recommendations.append(
                f"MA57 improved convergence in {convergence_improvements} cases",
            )

        if total_runs < 10:
            recommendations.append(
                "Collect more optimization runs for better statistical confidence",
            )
    ...
    def _determine_migration_priority(
        ...
        if (
            beneficial_percentage > 0.7
            and (average_speedup is None or average_speedup > 1.3)
            and convergence_improvements > 0
        ):
            return "high"

        # Medium priority conditions
        if (
            beneficial_percentage > 0.4
            or (average_speedup and average_speedup > 1.5)
            or convergence_improvements > total_runs * 0.2
        ):
            return "medium"
    ...
    def _get_phase_priorities(
        ...
        for phase, analysis in phase_analysis.items():
            if analysis["count"] > 0:
                priority = (
                    "high"
                    if analysis["beneficial_percentage"] > 0.6
                    else "medium"
                    if analysis["beneficial_percentage"] > 0.3
                    else "low"
                )
                phases.append(
                    {
