...
class ValidationStatisticsCollector:
    ...
    def __init__(self, output_dir: Optional[Path] = None):
        ...
        log.info(f"Validation statistics collector initialized. Output directory: {self.output_dir}")
    ...
    def compute_statistics(self) -> ValidationStatistics:
        ...
        successful_validations = sum(1 for m in self.metrics if m.python_converged and m.casadi_converged)
        ...
        # Performance statistics
        evaluation_speedups = [m.evaluation_speedup for m in self.metrics if m.evaluation_speedup > 0]
        gradient_speedups = [m.gradient_speedup for m in self.metrics if m.gradient_speedup > 0]
        ...
        problem_type_success_rates = {
            ...
            for ptype, (total_count, success_count) in 
            zip(problem_types.keys(), zip(problem_types.values(), problem_type_success.values()))
        }
        ...
        collection_duration = (max(timestamps) - min(timestamps)).total_seconds() / (24 * 3600)
...
"""
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tolerance_success_rate": sum(1 for m in self.metrics if m.within_tolerance) / len(self.metrics)
        }
