...
from campro.optimization.solver_selection import ProblemCharacteristics, AnalysisHistory
...
class DynamicParameterTuner:
    ...
    def tune_parameters(self, phase: str, problem_chars: ProblemCharacteristics,
                       analysis_history: Optional[AnalysisHistory]) -> SolverParameters:
        ...
        log.debug(f"Tuned parameters for {phase} phase: max_iter={params.max_iter}, "
    ...
    def estimate_problem_characteristics(self, n_variables: int, n_constraints: int, 
                                       phase: str) -> ProblemCharacteristics:
    ...
    def tune_for_phase(self, phase: str, n_variables: int, n_constraints: int,
                      analysis_history: Optional[AnalysisHistory] = None) -> SolverParameters:
        """Convenience method to tune parameters for a specific phase."""
        
        problem_chars = self.estimate_problem_characteristics(n_variables, n_constraints, phase)
        return self.tune_parameters(phase, problem_chars, analysis_history)
    
    def get_tuning_summary(self, phase: str, problem_chars: ProblemCharacteristics,
                          params: SolverParameters) -> dict:
        ...
        return {
            ...
            "tuning_rationale": self._get_tuning_rationale(phase, problem_chars, params)
        }
    
    def _get_tuning_rationale(self, phase: str, problem_chars: ProblemCharacteristics,
                             params: SolverParameters) -> List[str]:
        """Get rationale for parameter tuning decisions."""
        rationale = []
        
        if params.max_iter > self.default_params.max_iter:
            rationale.append(f"Increased max_iter to {params.max_iter} for {phase} phase")
        ...
        if params.mu_strategy != self.default_params.mu_strategy:
            rationale.append(f"Changed mu_strategy to {params.mu_strategy} for convergence issues")
