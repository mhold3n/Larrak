...
from provenance.spec import RunStartEvent, RunEndEvent
from provenance.db import db, ProvenanceDB
...
def _set_current_run_id(run_id: Optional[str]):
    _local.run_id = run_id

class RunContext:
    def __init__(self, module_id: str, args: Optional[List[str]] = None, capture_output: bool = False):
    ...
    def start(self):
        ...
        print(f"[Provenance] Started Run {self.run_id} for Module {self.module_id}")
        return self.run_id
