...
def generate_migration_plan(
    ...
    migration_plan = {
        "executive_summary": {
            ...
            "beneficial_percentage": analysis.ma57_beneficial_runs / analysis.total_runs if analysis.total_runs > 0 else 0,
    ...
    migration_plan["implementation_timeline"] = generate_implementation_timeline(analysis.migration_priority)
...
def generate_risk_assessment(analysis) -> Dict[str, Any]:
    ...
    return {
        "risk_level": "low" if analysis.migration_priority == "low" else "medium" if analysis.migration_priority == "medium" else "high",
        "risks": risks,
...
def generate_cost_benefit_analysis(analysis) -> Dict[str, Any]:
    ...
    # Benefits
    if analysis.average_speedup and analysis.average_speedup > 1.2:
        benefits.append(f"Performance improvement of {analysis.average_speedup:.1f}x on average")
    
    if analysis.convergence_improvements > 0:
        benefits.append(f"Improved convergence in {analysis.convergence_improvements} cases")
    ...
    # ROI calculation
    roi_estimate = "positive" if analysis.migration_priority in ["high", "medium"] else "neutral"
    
    return {
        "costs": costs,
        "benefits": benefits,
        "roi_estimate": roi_estimate,
        "payback_period": "3-6 months" if analysis.migration_priority == "high" else "6-12 months" if analysis.migration_priority == "medium" else "12+ months"
    }
...
def print_plan_summary(plan: Dict[str, Any]):
    ...
    for i, step in enumerate(plan["implementation_roadmap"]["implementation_steps"][:3], 1):
        print(f"  {i}. {step}")
