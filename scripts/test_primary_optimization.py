...
def print_result_details(result, test_name):
    ...
    if hasattr(result, 'solution') and result.solution:
        ...
        if 'cam_angle' in result.solution:
            ...
            print(f"Cam Angle Shape: {cam_angle.shape if hasattr(cam_angle, 'shape') else len(cam_angle)}")
        
        if 'position' in result.solution:
            position = result.solution['position']
            print(f"Position Shape: {position.shape if hasattr(position, 'shape') else len(position)}")
        
        if 'velocity' in result.solution:
            velocity = result.solution['velocity']
            print(f"Velocity Shape: {velocity.shape if hasattr(velocity, 'shape') else len(velocity)}")
...
def test_primary_optimization_simple():
    ...
    print(f"  - upstroke_duration_percent: {input_data['motion_law']['upstroke_duration_percent']}%")
    print(f"  - zero_accel_duration_percent: {input_data['motion_law']['zero_accel_duration_percent']}%")
    ...
    try:
        ...
        print(f"Data after input update:")
        print(f"  - stroke: {framework.data.stroke}")
        print(f"  - upstroke_duration_percent: {framework.data.upstroke_duration_percent}")
        print(f"  - zero_accel_duration_percent: {framework.data.zero_accel_duration_percent}")
        ...
        print(f"Data after primary update:")
        ...
        print(f"  - primary_acceleration: {framework.data.primary_acceleration is not None}")
...
def main():
    ...
    if simple_result and thermal_result:
        ...
        if simple_result.is_successful() and thermal_result.is_successful():
            ...
            print("  🔍 Issue likely in cascaded optimization flow, not primary optimization")
