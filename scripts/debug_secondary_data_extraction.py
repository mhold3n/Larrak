...
def debug_secondary_data_extraction():
    ...
    try:
        ...
        print(f"Step 1: Running primary optimization...")
        ...
        print(f"Primary data set: theta={framework.data.primary_theta is not None}, position={framework.data.primary_position is not None}")
        ...
        if hasattr(secondary_result, 'metadata') and secondary_result.metadata:
            ...
            if 'optimized_gear_config' in secondary_result.metadata:
                ...
                if isinstance(gear_config, dict) and 'base_center_radius' in gear_config:
        ...
        if secondary_result.status.value == "CONVERGED":
            ...
            if base_radius is not None:
                ...
                print(f"❌ Base radius extraction failed")
                ...
                if hasattr(secondary_result, 'metadata') and secondary_result.metadata:
                    ...
                    if 'optimized_gear_config' in metadata:
                        ...
                        if isinstance(gear_config, dict):
                            ...
                            print(f"Alternative base_radius from metadata: {alt_base_radius}")
        ...
        print(f"Before update - secondary_base_radius: {framework.data.secondary_base_radius}")
        
        framework._update_data_from_secondary(secondary_result)
        
        print(f"After update - secondary_base_radius: {framework.data.secondary_base_radius}")
        
        if framework.data.secondary_base_radius is not None:
            print(f"✅ _update_data_from_secondary worked correctly")
        else:
            print(f"❌ _update_data_from_secondary failed to set secondary_base_radius")
