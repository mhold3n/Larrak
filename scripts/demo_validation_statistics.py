...
def generate_realistic_validation_metrics(problem_id: str, problem_type: str, 
                                        stroke: float, cycle_time: float) -> ValidationMetrics:
    """
    ...
    torque_avg_rel_diff = abs(torque_avg_diff) / abs(python_torque_avg) if python_torque_avg != 0 else 0
    torque_ripple_rel_diff = abs(torque_ripple_diff) / abs(python_torque_ripple) if python_torque_ripple != 0 else 0
    side_load_penalty_rel_diff = abs(side_load_penalty_diff) / abs(python_side_load_penalty) if python_side_load_penalty != 0 else 0
    litvin_objective_rel_diff = abs(litvin_objective_diff) / abs(python_litvin_objective) if python_litvin_objective != 0 else 0
    litvin_closure_rel_diff = abs(litvin_closure_diff) / abs(python_litvin_closure) if python_litvin_closure != 0 else 0
    ...
    max_rel_diff = max(torque_avg_rel_diff, torque_ripple_rel_diff, side_load_penalty_rel_diff,
                      litvin_objective_rel_diff, litvin_closure_rel_diff)
    within_tolerance = max_rel_diff < 1e-4  # Should be within tolerance with small variations
...
def run_validation_demo():
    ...
    print(f"📊 Collecting validation metrics...")
    ...
    for i in range(total_problems):
        ...
        metrics = generate_realistic_validation_metrics(
            problem_id, problem_type, stroke, cycle_time
        )
    ...
    print(f"Successful Validations: {statistics.successful_validations} ({statistics.successful_validations/statistics.total_validations*100:.1f}%)")
    print(f"Within Tolerance: {statistics.within_tolerance_count} ({statistics.tolerance_success_rate*100:.1f}%)")
    ...
    print(f"Evaluation Speedup - Mean: {statistics.evaluation_speedup_stats['mean']:.2f}x")
    print(f"Evaluation Speedup - Median: {statistics.evaluation_speedup_stats['median']:.2f}x")
    print(f"Gradient Speedup - Mean: {statistics.gradient_speedup_stats['mean']:.2f}x")
    print(f"Gradient Speedup - Median: {statistics.gradient_speedup_stats['median']:.2f}x")
    ...
    for problem_type, count in statistics.problem_type_counts.items():
        ...
        print(f"{problem_type}: {count} problems, {success_rate*100:.1f}% success rate")
    ...
    if statistics.tolerance_success_rate >= 0.95:
        ...
        print("⚠️  CONDITIONAL ENABLING: CasADi physics mostly meets criteria but needs monitoring.")
    else:
