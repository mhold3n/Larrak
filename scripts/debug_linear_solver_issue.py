...
from campro.optimization.ipopt_factory import (
    create_ipopt_solver, 
...
def test_validation_process():
    ...
    # Reset the global flag
    reset_linear_solver_flag()
    print(f"Linear solver initialized before validation: {is_linear_solver_initialized()}")
    ...
    try:
        ...
        print(f"Linear solver initialized after validation: {is_linear_solver_initialized()}")
...
def test_unified_framework_initialization():
    ...
    # Reset the global flag
    reset_linear_solver_flag()
    print(f"Linear solver initialized before framework: {is_linear_solver_initialized()}")
    ...
    try:
        ...
        print(f"Linear solver initialized after framework: {is_linear_solver_initialized()}")
...
def test_optimization_with_tracing():
    ...
    # Reset the global flag
    reset_linear_solver_flag()
    print(f"Linear solver initialized before optimization: {is_linear_solver_initialized()}")
    
    try:
        # Create framework
        settings = UnifiedOptimizationSettings()
        settings.use_thermal_efficiency = True  # Use thermal efficiency to trigger the issue
        framework = UnifiedOptimizationFramework('TestFramework', settings)
        
        print(f"Linear solver initialized after framework creation: {is_linear_solver_initialized()}")
        ...
        print(f"Linear solver initialized before primary optimization: {is_linear_solver_initialized()}")
        
        # Run primary optimization
        primary_result = framework._optimize_primary()
        
        print(f"Linear solver initialized after primary optimization: {is_linear_solver_initialized()}")
...
def main():
    ...
    print("This script traces where an unexpected linear solver is set instead of MA27/MA57")
