...
def cmd_solve(args: argparse.Namespace) -> int:
    ...
    if args.diagnose:
        ...
        try:
            ipopt_log = (
                report.artifacts.get("ipopt_log")
                if hasattr(report, "artifacts")
                else None
            )
        except Exception:
            ipopt_log = None
    ...
    if args.export_gear:
        print(
            "--export-gear requested, but gear synthesis/export is not implemented yet.",
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_solve.add_argument(
        "--spec", required=True, help="Path to YAML/JSON ProblemSpec file",
    )
    p_solve.add_argument(
        "--diagnose", action="store_true", help="Run Ipopt log diagnostics",
    )
    p_solve.add_argument(
        "--export-gear", action="store_true", help="Stub: export gear DXF",
    )
