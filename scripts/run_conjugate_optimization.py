...
from surrogate.conjugate_optimizer import ConjugateOptimizer
...
def run_conjugate_opt():
    ...
    fig_r.update_layout(title="Optimized Gear Radii vs Carrier Angle (2-Cycle 2:1)", yaxis_title="Radius [m]")
    ...
    fig_polar = make_subplots(rows=1, cols=2, specs=[[{'type': 'polar'}, {'type': 'polar'}]],
                              subplot_titles=["Ring Pitch Curve", "Planet Pitch Curve"])
    
    fig_polar.add_trace(go.Scatterpolar(
        r=Rr, theta=np.degrees(theta_2cycle), # Approx mapping
        mode='lines', name='Ring', fill='toself'
    ), row=1, col=1)
    
    fig_polar.add_trace(go.Scatterpolar(
        r=Rp, theta=np.degrees(psi_wrap), # Mapped to Planet Spin
        mode='lines', name='Planet', fill='toself'
    ), row=1, col=2)
    
    fig_polar.update_layout(title="Conjugate Pitch Shapes", template="plotly_dark")
    
    # Save
    fig_r.write_html(os.path.join(PROJECT_ROOT, "dashboard/conjugate_radii.html"))
    fig_polar.write_html(os.path.join(PROJECT_ROOT, "dashboard/conjugate_shapes.html"))
