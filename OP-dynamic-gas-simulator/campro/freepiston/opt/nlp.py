...
def piston_force_balance(
    *,
    ...
    F_clearance = (
        k_clearance * gap_violation * ca.tanh(gap_violation / clearance_smooth)
    )
    ...
    for k in range(K):
        ...
        for j in range(C):
            ...
            m_total_L = geometry.get("mass", 1.0) + geometry.get("rod_mass", 0.5) * (
                geometry.get("rod_cg_offset", 0.075) / geometry.get("rod_length", 0.15)
            )
...
def build_collocation_nlp(P: Dict[str, Any]) -> Tuple[Any, Dict[str, Any]]:
    ...
    for k in range(K):
        for j in range(C):
            ...
            # Combustion heat release should be positive and within bounds
            g += [Q_comb_kj]  # Combustion heat release constraint
            lbg += [0.0]  # No negative heat release
            ubg += [bounds.get("Q_comb_max", 10000.0)]

            # Combustion timing constraints (simplified)
            # In practice, this would be more sophisticated
            if Q_comb_kj > 0.0:
                # If combustion is active, ensure it's within timing bounds
                # This is a simplified constraint - in practice, you'd track crank angle
                g += [Q_comb_kj]  # Additional timing constraint
                lbg += [0.0]
                ubg += [bounds.get("Q_comb_max", 10000.0)]
