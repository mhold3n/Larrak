...
class OptimalMotionSolver:
    ...
    def _generate_simple_motion_law(
        ...
        # If time is fixed, scale distance to respect velocity/acceleration limits
        d_eff = distance
        if time_horizon is not None:
            scales = []
            if max_velocity is not None:
                vmax_coeff = 1.875  # v_max ≈ 1.875 d / T for min-jerk
        ...
        # Minimum-jerk 5th-order polynomial profile
        tau = t / T
        position = d_eff * (10.0 * tau**3 - 15.0 * tau**4 + 6.0 * tau**5)
        velocity = (d_eff / T) * (30.0 * tau**2 - 60.0 * tau**3 + 30.0 * tau**4)
        acceleration = (d_eff / T**2) * (60.0 * tau - 180.0 * tau**2 + 120.0 * tau**3)
        jerk = (d_eff / T**3) * (60.0 - 360.0 * tau + 360.0 * tau**2)

        # If no time_horizon, enforce constraints by stretching time only
        if time_horizon is None:
            vmax = np.max(np.abs(velocity))
            amax = np.max(np.abs(acceleration))
            jmax = np.max(np.abs(jerk))
            scales = []
            if max_velocity is not None and vmax > 1e-12:
                scales.append(max_velocity / vmax)
            if max_acceleration is not None and amax > 1e-12:
                scales.append(np.sqrt(max_acceleration / amax))
            if max_jerk is not None and jmax > 1e-12:
                scales.append(np.cbrt(max_jerk / jmax))
            if scales:
                scale_time = min(1.0, min(scales))
                if scale_time < 1.0:
                    T = T / scale_time
                    t = np.linspace(0.0, T, n_points)
                    tau = t / T
        ...
        # If time_horizon is provided, ensure final position equals requested distance
        # by linear offset without violating endpoint derivatives (already zero at endpoints).
        if time_horizon is not None:
